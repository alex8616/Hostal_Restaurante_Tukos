    ...
    <ul>
        ...
        <li>
            <a data-toggle="modal" data-target="#MostrarPdf{{ $comandaspdf->id }}">Imprimir Ticket</a>
                @include('admin.festival.MostrarPdfreserva')
            </a>
        </li>
        @endif
    </ul>
...
<form action="{{ route('admin.festival.storereservafestival') }}" method="post" enctype="multipart/form-data" class="registrar-form">
  @csrf
            ...
            <div class="card-body p-0">
                <div class="container">
                    <div class="mb-3">
                        <div class="form-row">
                            <div class="col-md-6">
                                <label for="inputNombre" class="is-required">Nombre De La Reserva</label>
                                <select class="select2" name="reserva_id" id="reserva_id">
                                    <option value="">Seleccione Una Reserva</option>
                                    @foreach($reservas as $reserva)
                                        <option value="{{$reserva->id}}">{{$reserva->Nombre_reserva}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="group">
                                    ...
                                    <input type="text" id="Cantidad_persona" name="Cantidad_persona" class="form-control input_style" placeholder="$ 0.00" autocomplete="off">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="group">
                                    <label for=""># Mesa</label>
                                    <input type="text" id="mesa_id" name="mesa_id" class="form-control input_style" readonly="" placeholder="$ 0.00">
                                </div>
                            </div><br><br><br><br>
                            <div class="col-md-4">
                                <div class="group">
                                    <label for="">Adelanto</label>
                                    <input type="text" id="Adeltanto_reserva" name="Adeltanto_reserva" class="form-control input_style" placeholder="$ 0.00">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="group">
                                    <label for="">Pago Realizado</label>
                                    <input type="text" id="Total_reserva" name="Total_reserva" class="form-control input_style" readonly="" placeholder="$ 0.00">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="group">
                                    <label for="">Pago Realizado</label>
                                    <input type="text" id="pagado" name="pagado" class="form-control input_style" readonly="" placeholder="$ 0.00">
                                </div>
                            </div><br><br><br><br>
                            <div class="col-md-12">
                                <div class="group">
                                    <input type="text" name="total" id="total_pagar" class="form-control text-center input_style_total" placeholder="00.00">
                                </div>
                            </div><br><br>
                            <div class="col-md-8">
                                <div class="group">
                                    <label for=""><span style="font-size: 25px; color:black;">PAGO A REALIZARSE</span></label>
                                </div>
                                <span style="color: #FF0000;">*Nota: el monto que se refleja es la diferencia del Adelanto que se Hizo anteriormente</span>
                            </div>
                            <div class="col-md-4">
                                <div class="group">
                                    <input type="text" name="total_request" id="total_request" class="form-control text-center input_style_total" placeholder="00.00">
                                </div>                                
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <input type="button" id="calcular_button" value="Calcular" hidden>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body p-0" id="table_infor" style="height: 330px; overflow-y: scroll;">
                <div class="container">
                    <table class="table table-hover">
                        ...
                        @foreach($reservafestivales as $festivale)
                            <tr>
                                <td>{{ $festivale->id }}</td>
                                <td>{{ $festivale->Nombre_reserva }}</td>
                                <td>{{ $festivale->Fecha_registro }}</td>
                                <td>
                                @foreach ($detallefestivales as $detallefestivale)
                                    @if($festivale->id == $detallefestivale->reservafestival_id)
                                ...
                                <td style="margin: 50%">
                                    <ul class="wrapper" style="height: 50px; display: inline-flex;">
                                        ...
                                        @include('admin.festival.tablereservapdf')
                                    </ul>
                                    <ul class="wrapper" style="height: 50px; display: inline-flex;">
                                        ...
                                        @include('admin.festival.add_detalle')
                                    </ul>
                                </td>
                            </tr>
                        @endforeach
...
<script>
  ...
  var adelantoReservaInput = document.getElementById('Adeltanto_reserva');
  ...
  // Agrega evento de clic al botón "calcular"
  calcularButton.addEventListener('click', function() {
    var adelantoReserva = parseFloat(adelantoReservaInput.value);
    var totalPagar = parseFloat(totalPagarInput.value);
    var totalRequest = adelantoReserva - totalPagar;

    // Actualiza el valor del campo "total_request"
    if(totalRequest.toFixed(2) < 0){
        totalRequestInput.value = totalRequest.toFixed(2)*-1; // Limita el resultado a dos decimales
    }else{
        totalRequestInput.value = 0; // Limita el resultado a dos decimales
    }
  });

  // Función que hace clic en el botón "calcular"
  function hacerClicEnCalcular() {
    calcularButton.click();
  }

  // Ejemplo de llamada a la función para hacer clic en el botón
  hacerClicEnCalcular();
</script>
<script>
    $(document).ready(function() {
        // Manejar el evento de cambio en el campo de selección de reserva
        $('#reserva_id').change(function() {
            calcularButton.click();
            var reservaId = $(this).val();

            $.ajax({
                url: '{{ route('admin.festival.obtenerDatosReserva') }}',
                method: 'GET',
                data: {reserva_id: reservaId},
                success: function(response) {    
                    console.log(response)                
                    // Actualizar los campos de entrada con los datos de la reserva
                    $('#Cantidad_persona').val(response.Cantidad_persona);
                    $('#mesa_id').val(response.mesa_id);
                    $('#Adeltanto_reserva').val(response.Adeltanto_reserva);
                    $('#Total_reserva').val(response.Total_reserva);
                    if(response.pagado == 'FALSE'){
                        $valor = 'NO'
                        $('#pagado').val($valor);
                    }else{
                        $valor = 'SI'
                ...
                error: function() {
                    ...
                    alert('Ocurrió un error al obtener los datos de la reserva.');
                }
            });
        });
    });
