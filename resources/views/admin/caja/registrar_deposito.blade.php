...
<br><br><hr>
<div class="header">
  <span class="header-text">CAJA MES {{ \Carbon\Carbon::parse($caja->fecha_registro)->locale('es')->isoFormat('MMMM \d\e\l Y') }} - DEPOSITO</span>
...
@include('admin.caja.CrearDetalleCajaDepositos')
<!-- Modal -->
...
<script>
    ...
    $('#categoria').DataTable({
        ...
        "ajax": "{{ route('detallecaja.datadepositos', ['caja' => $caja->id]) }}",
      ...
      var id = $(this).data('id');
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('caja.destroydetallecajaDepositos',['detallecaja' => ':id']) }}".replace(':id', id),
                    method: 'DELETE',
                    data: {
                        id: id
                    },
    ...
    $("#modalEdit .btn-primary").off('click').on("click", function() {
          $.ajax({
              type: 'put',
              url: "{{ route('detallecaja.updateDepositos', ['id' => ':id']) }}".replace(':id', $('input[name=id]').val()),
              data: {
                  '_token': "{{ csrf_token() }}",
