...
<br><br><hr>
<div class="header">
  <span class="header-text">CAJA MES {{ \Carbon\Carbon::parse($caja->fecha_registro)->locale('es')->isoFormat('MMMM \d\e\l Y') }} - HOSTAL</span>
  <nav class="dropdownmenu">
    <ul>      
      ...
      <li>
        <a href="#">REPORTES</a>
        <ul id="submenu">
            <li><a href="{{ route('admin.caja.reporte_mes_especifico', $caja) }}" target="_blank">REPORTE GENERAL</a></li>
        </ul>
      </li>
      <li>
      ...
      <li><a data-toggle="modal" data-target="#modalnovedades">NOVEDADES</a></li>
      <li><a data-toggle="modal" data-target="#modelId">AÑADIR</a></li>
      <li><a data-toggle="modal" data-target="#modalcerrarcaja" style="background: red; color: white">CERRAR CAJA</a></li>
    </ul>
  </nav>
</div>
...
<div style="width: 100%; padding: 20px; margin-top: -13px">
  <div style="background: white; width: 100%; height: auto; padding-left: 15px; padding-right: 15px; padding-top: 15px;">
    <table class="table table-bordered shadow-lg mt-4 dt-responsive" id="categoria" style="width: 100%;">
      ...
      <thead class="table-head" title="Click to sort">
        <tr class="table-row">
          ...
          <th class="col-head" scope="col" style="background: #277BC0; color: white"><center>Codigo</center></th>
    ...
    <div style="overflow-x: auto;">
      <table style="width: 100%">
        <tfoot id="my-tfoot">
            <tr>
              <td colspan="4" style="background-color:#F97B22; color:white; padding-top: 8px; padding-left: 15px"><a data-toggle="modal" data-target="#modeldolar"><strong>DOLAR</strong>:</a></td>
              <div class="modal fade" id="modeldolar" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title">INGRESAR DOLAR</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{ route('updatedolar', $caja->id) }}" method="POST" enctype="multipart/form-data">
                          <div class="modal-body">
                            <div class="container-fluid">
                              ...
                              <div class="form-group col-md-6">
                                  ...
                                  <input class="form-control" name="dolar" id="dolar" type="number" value="0.00">                                    
                              </div>
                              </div>
                            </div>
              ...
              <td colspan="1" style="background-color:#F97B22; color:white; padding-top: 8px; padding-left: 15px">${{ $caja->DolarHostal }}</td>
              <td colspan="1" style="background-color:#e95747; color:white; padding-top: 15px; padding-left: 15px"><h4><strong>CAJA GRANDE: {{ $cajagrande }}</strong></h4></td>
              <td colspan="2" style="background-color:#70ca58; color:white; padding-top: 15px; padding-left: 15px"><h4><strong>INGRESO TOTAL: {{ $detalleIngreso }}</strong></h4></td>
              <td colspan="2" style="background-color:#e95747; color:white; padding-top: 15px; padding-left: 15px"><h4><strong>EGRESO TOTAL: {{ $detalleEgreso }}</strong></h4></td>
              <td colspan="2" style="background-color:#52abff; color:white; padding-top: 15px; padding-left: 15px"><h4><strong>TOTAL: {{$total}}</strong></h4></td>
            </tr>
          </tfoot>
      </table>
    </div>
...
@include('admin.caja.CrearDetalleCaja')
@include('admin.caja.ModalCerrarCaja')
@include('admin.caja.ModalNovedades')
<!-- Modal -->
...
<script>
    ...
    $('#categoria').DataTable({
        ...
        "ajax": "{{ route('detallecaja.data', ['caja' => $caja->id]) }}",
      ...
      $('#add-form').on('submit', function(e) {
          ...
          $.ajax({
              ...
              success: function(data) {
                ...
                <tr>
                    <td colspan="5" style="background-color:#70ca58; color:white"><h4><strong>INGRESO TOTAL: ${data.totals.detalleIngreso}</strong></h4></td>
      ...
      var id = $(this).data('id');
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('caja.destroydetallecaja',['detallecaja' => ':id']) }}".replace(':id', id),
                    method: 'DELETE',
                    data: {
                        id: id
                    },
    ...
    $("#modalEdit .btn-primary").off('click').on("click", function() {
          $.ajax({
              type: 'put',
              url: "{{ route('detallecaja.update', ['id' => ':id']) }}".replace(':id', $('input[name=id]').val()),
              data: {
                  '_token': "{{ csrf_token() }}",
              ...
              success: function(data) {
                  ...
                  $('#my-tfoot').html(`<tr>
                      <td colspan="8"></td>
                      <td colspan="3" style="background-color:#e95747; color:white;"><h4><strong>CAJA GRANDE: ${data.totals.cajagrande}</strong></h4></td>
                  </tr>
                  <tr>
                      <td colspan="5" style="background-color:#70ca58; color:white"><h4><strong>INGRESO TOTAL: ${data.totals.detalleIngreso}</strong></h4></td>
                      <td colspan="4" style="background-color:#e95747; color:white"><h4><strong>EGRESO TOTAL: ${data.totals.detalleEgreso}</strong></h4></td>
