...
<form action="{{ route('admin.comanda.storecafeteria') }}" method="post" enctype="multipart/form-data" class="registrar-form">
  @csrf
                  ...
                  <div class="col-md-12">
                      ...
                      <input type="text" id="caja_id" name="caja_id" value="{{ $ultimo_registro->id }}" hidden>
                        <select name="id_plato" id="id_plato" class="form-select custom-select" size="6" style="width: 100%;">
                          ...
                          <center><optgroup label="C A F E T E R I A" class="column"></center>
                            @foreach ($platos as $index => $plato)
                              @if ($plato->Nombre_categoria == 'CAFETERIA')
                          ...
                          <center><optgroup label="M A S I T A S" class="column"></center>
                            @foreach ($platos as $index => $plato)
                              @if ($plato->Nombre_categoria == 'MASITAS')
                                <option value="{{ $plato->id }}_{{ $plato->stock }}_{{ $plato->Precio_plato }}">
                                  {{ $plato->Nombre_plato }}
                                </option>
                              @endif
