        ...
        <div class="col-md-12">        
            <div class="card div_radius">
                ...
                <div class="alert alert--danger">
                    <input type="text" id="AdelantoHospedaje" name="AdelantoHospedaje" value="{{$reservas->Adelanto}}" style="color:black;" hidden>
                    <input  class="buton" type="button" id="boton_calc" value="Costo" hidden>
                    @if($reservas->Adelanto == 0)
                    ...
                    @else
                        <h3>El Cliente Dejo Un Adelanto de <strong><input type="text" id="AdelantoHospedaje" name="AdelantoHospedaje" value="{{$reservas->Adelanto}}" style="color:black; width: 90px;" readonly></strong> 
                ...
                <input type="text" id="reserva_habitacion_id" name="reserva_habitacion_id" value="{{$reservas->id}}" hidden>
                <style>
                    .alert {
                        margin:auto;
                        background: white;
                    ...
                    <table class="table table-bordered table-sm table-hover __web-inspector-hide-shortcut__">
                        ...
                        @foreach($detallereservas as $detallereserva)
                            @foreach($clientes as $cliente)
                                @if($detallereserva->cliente_id == $cliente->id && $detallereserva->reserva_habitacion_id == $reservas->id)
                                <input class="typeahead form-control" id="hospedaje_habitacion_id" type="text" 
                                name="hospedaje_habitacion_id" value="{{$detallereserva->hospedaje_habitacion_id}}" hidden>
                                    <tr>
                        ...
                        <tr>
                            ...
                            @include('hostal.habitaciones.CrearReservaInvitado')
                            </td>
                        </tr>
                        @if($contInvitado > 0)
                            <table class="table table-bordered table-sm table-hover __web-inspector-hide-shortcut__">
                                ...
                                <tbody id="tabla_venta_productos_temp"> 
                                    @foreach ($full_reservas as $full_reserva)
                                        @if($full_reserva->reserva_habitacion_id == $reservas->id)
                                            @php
                                                $invitado_total = $full_reserva->invitado_Total;
                                                $pagado = $full_reserva->Pagado;
                                            @endphp                                               
                                            @foreach ($full_reserva->detalles as $index => $detalle)
                                                @if($full_reserva->Pagado == 'NO')                                                    
                                                <tr>
                                                    ...
                                                    <td>{{ $full_reserva->invitado_ingreso_reserva }}</td>
                                                    @if ($index === 0)
                                                        <td rowspan="{{ count($full_reserva->detalles) }}">{{ $invitado_total }}</td>
                                                        <td rowspan="{{ count($full_reserva->detalles) }}">{{ $pagado }}</td>
                                                    @endif                                                       
                                                </tr>
                                                @else
                                                <tr>
                                                    ...
                                                    <td><del>{{ $full_reserva->invitado_ingreso_reserva }}</del></td>
                                                    @if ($index === 0)
                                                        <td rowspan="{{ count($full_reserva->detalles) }}"><del>{{ $invitado_total }}</del></td>
                                                        <td rowspan="{{ count($full_reserva->detalles) }}"><del>{{ $pagado }}</del></td>
                                                    @endif                                                        
                                                </tr>
                                                @endif
                                            @endforeach
                        ...
                        <table class="table table-bordered table-sm table-hover __web-inspector-hide-shortcut__">
                            ...
                            <tr>
                                <td>
                                    <strong>INGRESO FECHA:</strong> {{ \Carbon\Carbon::parse($reservas->ingreso_hospedaje)->format('d-m-Y') }}
                                </td>
                                <td>
                                    <strong>INGRESO HORA:</strong> {{ \Carbon\Carbon::parse($reservas->created_at)->format('H:i a') }}
                                </td>
                                <td>
                                    <strong>SALIDA FECHA:</strong> {{ \Carbon\Carbon::parse($reservas->salida_hospedaje)->format('d-m-Y') }}
                                </td>
                                <td>
                                    <strong>TOTAL DIAS:</strong> {{$reservas->dias_reserva}}
                                </td>
                            </tr>
                            <tr>
                                <td colspan="2">
                                    <strong>PROCEDENTE DE:</strong> {{$reservas->procedencia_reserva}}
                                </td>
                                <td colspan="2">
                                    <strong>DESTINO:</strong> {{$reservas->destino_reserva}}
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <strong>PRECIO DE HABITACION:</strong> {{$reservas->Precio_habitacion_reserva}}
                                </td>
                                <td>
                                    @if($reservas->Adelanto_reserva > 0)
                                        <strong style="color:green">SE DEJO UN ADELANTO DE:</strong> {{$reservas->Adelanto_reserva}}
                                    @else
                                        <strong style="color:red">NO SE DEJO NINGUN ADELANTO</strong>
                                    @endif
                                </td>
                                <td>
                                    @if($reservas->PrecioRestante_reserva > 0)
                                        <strong style="color:red">DEUDA A PAGAR:</strong> {{$reservas->PrecioRestante_reserva}}
                                    @else
                                        <strong style="color:green">NO TIENE NINGUNA DEUDA</strong>
                                    @endif
                                </td>
                                <td>
                                    <strong>TOTAL:</strong> <input type="text" id="totalhospedaje" name="totalhospedaje" value="{{$reservas->Total_reserva}}">
                                </td>
                            </tr>
                        </table>
                        <table class="table table-bordered table-sm table-hover __web-inspector-hide-shortcut__">
                            ...
                            <tbody id="tabla_venta_productos_temp">
                                ...
                                @foreach($detalleproductos as $detalleproducto)
                                    @if($reservas->id == $detalleproducto->reserva_habitacions)
                                    <tr>
                                        <td>{{$i++}}</td>
                            ...
                            <tfoot>
                                <tr>
                                    <th style="text-align: right" colspan="6">TOTAL:  <input type="text" id="totalproduct" name="totalproduct" value="{{$reservas->TotalProducto_reserva}}"></th>
                                </tr>
                            </tfoot>
                        </table>
                        <table class="table table-bordered table-sm table-hover __web-inspector-hide-shortcut__">
                            ...
                            <tbody id="tabla_venta_productos_temp">
                                ...
                                @foreach($detalleservicios as $detalleservicio)
                                    @if($reservas->id == $detalleservicio->reserva_habitacion_id)
                            ...
                            <tfoot>
                                <tr>
                                    <th style="text-align: right" colspan="7">TOTAL:  <input type="text" id="TotalServicioSum" name="TotalServicioSum" value="{{$reservas->TotalServicio_reserva}}"></th>
                                </tr>
                            </tfoot>
                        </table>
                    </table>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" action="{{ route('updatehabitacion', $habitacion->id) }}">
                                        ...
                                        <input type="text" id="Hospedaje_id" name="Hospedaje_id" hidden>
                                        <input type="text" id="Reserva_id" name="Reserva_id" value="{{$reservas->id}}" hidden>

                                        <label for="">Tipo De Pago a Realizarse</label>
                                        <div class="payment-cards">
                                            <div class="payment-card">
                                                <input type="radio" id="payment1" name="payment" value="EFECTIVO" style="pointer-events: none;">
                                                <label for="payment1">
                                            ...
                                            <div class="payment-card">
                                                <input type="radio" id="payment2" name="payment" value="TARJETA" style="pointer-events: none;">
                                                <label for="payment2">
                                            ...
                                            <div class="payment-card">
                                                <input type="radio" id="payment3" name="payment" value="DEPOSITO" style="pointer-events: none;">
                                                <label for="payment3">
...
@include('hostal.habitaciones.ServicioDesayunoReserva')
@include('hostal.habitaciones.ServicioLimpiezaReserva')
    ...
    <div class="modal fade" id="modelServicio" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input class="typeahead form-control" id="reserva_habitacion_id" type="text" 
                            name="reserva_habitacion_id" value="{{$reservas->id}}" hidden>
                    <input type="text" id="user_id" name="user_id" value="{{Auth::user()->id}}" hidden>
                    <div class="form-row" style="margin: 10px;">
                        <div class="col-md-3">
                            <label for="inputNombre"  class="is-required">Nombre Del Servicio</label><br>
                            <select name="servicio_hostals_id" id="servicio_hostals_id" size="3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="datetime" id="FechaRegistro_servicio_extra" name="FechaRegistro_servicio_extra" value="<?php echo date("Y-m-d H:i:s") ?>" readonly>
                        </div>
                        <div class="col-md-2">
                            <label for="cantidad_servicio_extra">Cantidad</label>
                            <input class="typeahead form-control" type="text" id="cantidad_servicio_extra" name="cantidad_servicio_extra" oninput="sum()">
                        </div>
                        <div class="col-md-2">
                            <label for="Precio_servicio_extra">Precio Servicio</label>
                            <input class="typeahead form-control" type="text" id="Precio_servicio_extra" name="Precio_servicio_extra" oninput="sum()">
                        </div>
                        <div class="col-md-2">
                            <label for="TotalServicio_extra">Total A Pagar</label>
                            <input class="typeahead form-control" type="text" id="TotalServicio_extra" name="TotalServicio_extra">
                            <script>
                                try {function sum() {
...
<script>
    document.getElementById("ConcluirReserva").disabled = true;

    window.addEventListener("load", function() {
    document.getElementById("ConcluirReserva").disabled = false;
    });
</script>
<script>
    ...
    function res() {
        ...
        document.getElementById("SumTotal").value=SumTotalFull.toFixed(2);
    }
    var tiempo = 1000;
    // intervalo
...
<script>
    ...
    function agregar_product() {
        ...
        datosTipoPagoProduct = $("#pagado").val();
