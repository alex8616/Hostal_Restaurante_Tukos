        ...
        <div style="min-width: 600px">
            ...
            <main>
                <div class="row contacts">
                            ...
                            <table class="table" style="font-size: 11px;">
                                ...
                                @foreach($clientes as $cliente)
                                    @foreach($detallereservas as $detallereserva)
                                        @if($cliente->id == $detallereserva->cliente_id)
                                        <tr>
                                            <td>
                    ...
                    <div class="col invoice-details"><br>
                        <h1 class="invoice-id">{{$reservas->Nombre_habitacion}} - 
                            @foreach($categorias as $categoria)
                                @if($categoria->id == $reservas->categoria_id)
                        ...
                        <table id="newtable" style="font-size: 14px;">
                            <tr style="background: red;">
                                <td>
                                    <strong>Procedencia:</strong> {{$reservas->procedencia_reserva}}
                                    <br><br><strong>Destino:</strong> {{$reservas->destino_reserva}}
                                </td>
                                <td colspan="" style="width: 250px;">
                                </td>
                                <td>
                                    <strong>Fecha Ingreso:</strong> {{$reservas->ingreso_reserva}}
                                    <br><br><strong>Fecha Salida:</strong> {{$reservas->salida_reserva}}
                                </td>
                            </tr>                           
                        </table>                        
                    </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" style="background-color: #4597d5; border: 1px solid; text-align:left;"># Noches</th>
                            <th scope="col" style="background-color: #4597d5; border: 1px solid; text-align:left;">Precio Noche</th>
                            <th scope="col" style="background-color: #4597d5; border: 1px solid; text-align:left;">Total</th>                
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td colspan="2">{{$reservas->Nombre_habitacion}}</td>
                            <td>{{intval($reservas->dias_reserva)}}</td>
                            <td>{{$reservas->Precio_habitacion_reserva}}</td>
                            <th style="background-color: #4597d5; border: 1px solid;">{{$reservas->Total_reserva}}</th>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            @foreach($detalleservicios as $detalleservicio)
                                @if($detalleservicio->reserva_habitacion_id == $reservas->reserva_habitacion_id && $detalleservicio->Incluye_servicio == 'NO')
                                <tr>
                                    ...
                                    <th style="background-color: #4597d5; border: 1px solid;">{{$detalleservicio->cantidad_servicio * $detalleservicio->Precio_servicio}}</th>
                                </tr>
                                @endif
                            @endforeach
                        @if($NProducto > 0)
                        ...
                        @foreach($productos as $producto)
                            @if($producto->reserva_habitacion_id == $reservas->reserva_habitacion_id && $producto->Tipo_pagado == 'Por Paga')
                            <tr>
                                <td>{{$producto->Nombre_producto}}</td>
                                <td>{{$producto->Tipo_pagado}}</td>
                                <td>{{$producto->cantidad}}</td>
                                <td>{{$producto->Precio_venta}}</td>
                                <th style="background-color: #4597d5; border: 1px solid;">{{$producto->cantidad * $producto->Precio_venta}}</th>
                            </tr>
                            @endif
                            @if($producto->reserva_habitacion_id == $reservas->reserva_habitacion_id && $producto->Tipo_pagado == 'Pagado')
                            <tr>
                                <td>{{$producto->Nombre_producto}}</td>
                                <td>{{$producto->Tipo_pagado}}</td>
                                <td>{{$producto->cantidad}}</td>
                        ...
                        @endif                            
                        <tr>
                            <th colspan="4" style="background-color: #4597d5;"><center>TOTAL GENERAL</center></th>
                            <th style="background-color: #4597d5; border: 1px solid;">{{$reservas->TotalGeneralHospedaje_reserva + $reservas->Adelanto_reserva}}</th>
                        </tr>
                    </tbody>
                </table>                   
                </footer><br>
