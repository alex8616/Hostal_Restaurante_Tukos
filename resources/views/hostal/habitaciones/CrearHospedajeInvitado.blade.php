...
<div class="modal fade" id="modelId" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document" style="width: 1000px; height: 1100px;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form onsubmit="validar();" action="{{ route('hostal.habitacion.storeclienteinvitadoHospedaje') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-row" style="margin: 10px;">
                        <div class="col-md-6">
                            <label for="invitado_ingreso_hospedaje"  class="is-required">Ingreso</label>
                            <input class="otraclase" id="invitado_ingreso_hospedaje" type="date" 
                            name="invitado_ingreso_hospedaje" min="<?php echo date('Y-m-d') ?>" oninput="calculate()">
                        </div>
                        <div class="col-md-6">
                            <label for="invitado_salida_hospedaje">Salida</label>
                            <input class="otraclase" name="invitado_salida_hospedaje" id="invitado_salida_hospedaje" type="date" min="<?php echo date('Y-m-d') ?>" oninput="calculate()">
                        </div>
                    </div>
                    <div class="form-row" style="margin: 10px;">
                        <div class="col-md-6">
                            <label for="pagado"  class="is-required">Estado De Pago</label>
                            <select class="otraclase" name="pagado" id="pagado">
                                <option value="SI">Pagado</option>
                                <option value="NO">POR Pagar</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label for="invitado_Total"  class="is-required">Monto a pagar</label>
                            <input class="otraclase" name="invitado_Total" id="invitado_Total" type="text">
                        </div>
                    </div>
                    <button class="btn btn-success" type="submit" style="width: 100%">REGISTRAR</button>
                </form>
            </div>
        </div>
    </div>
