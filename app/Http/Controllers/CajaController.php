...
{       
    ...
    public function estadoResultado(Request $request){
        ...
        $FinalFecha = $request->FinalFecha;
        $cajas = Caja::whereBetween('fecha_registro', [$InicioFecha, $FinalFecha])->get();
        
        $cajasWithSum = [];
        
        foreach ($cajas as $caja) {
            $CajaGrandeHostal = DetalleCaja::where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 2)
                ->where('articulo_caja_id', 29)
                ->sum('Egreso');
                
            $CajaGrandeRestaurante = DetalleCaja::where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 1)
                ->where('articulo_caja_id', 29)
                ->sum('Egreso');
                
            $IngresoRestaurante = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Ingreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 1)
                ->groupBy('articulo_caja_id', 'articulo_cajas.Nombre_articulo')
                ->havingRaw('SUM(Ingreso) > 0')
                ->get();

            $EgresoRestaurante = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Egreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 1)
                ->groupBy('articulo_caja_id', 'articulo_cajas.Nombre_articulo')
                ->havingRaw('SUM(Egreso) > 0')
                ->get();

            $IngresoHostal = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Ingreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 2)
                ->groupBy('articulo_caja_id', 'articulo_cajas.Nombre_articulo')
            ...
            $EgresoHostal = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Egreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 2)
                ->groupBy('articulo_caja_id', 'articulo_cajas.Nombre_articulo')
            ...
            $IngresoTarjeta = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Ingreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
            ...
            $EgresoTarjeta = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Egreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 3)
                ->groupBy('articulo_caja_id', 'articulo_cajas.Nombre_articulo')
            ...
            $IngresoDeposito = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Ingreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
            ...
            $EgresoDeposito = DetalleCaja::select('articulo_cajas.Nombre_articulo', \DB::raw('SUM(Egreso) as total'))
                ->join('articulo_cajas', 'detalle_cajas.articulo_caja_id', '=', 'articulo_cajas.id')
                ->where('caja_id', $caja->id)
                ->where('detalle_cajas.codigo_caja_id', '=', 4)
                ->groupBy('articulo_caja_id', 'articulo_cajas.Nombre_articulo')
            ...
            $cajasWithSum[] = [
                'caja' => $caja,
                '$CajaGrandeHostal' => $CajaGrandeHostal,
                '$CajaGrandeRestaurante' => $CajaGrandeRestaurante,
                '$IngresoRestaurante' => $IngresoRestaurante,
                '$EgresoRestaurante' => $EgresoRestaurante,
                '$IngresoHostal' => $IngresoHostal,
                '$EgresoHostal' => $EgresoHostal,
        ...
        //return view('admin.caja.reporte_general_mensual', compact('cajasWithSum'));
        //return response()->json($cajasWithSum);
        $pdf = PDF::loadView('admin.caja.reporte_general_mensual',
                     compact('cajasWithSum'));
        return $pdf->stream('ReporteMes'.$caja->id.'pdf');
        
        /*
        
        $fecha = new DateTime($caja->InicioFecha);
        ...
        $pdf = PDF::loadView('admin.caja.reporte_mes_especifico',['diasDelMes' => $diasDelMes, 'registrosPorDia' => $registrosPorDia],
                     compact('caja', 'CajaGrandeHostal', 'CajaGrandeRestaurante','codigos'));
        return $pdf->stream('ReporteMes'.$caja->id.'pdf');
        */
    }
    ...
    public function index(){
        ...
        foreach ($cajas as $caja) {
            // Convert the values to floats to handle decimal numbers
            $caja_hostal_ingreso = floatval($caja->caja_hostal_ingreso);
            $caja_restaurante_ingreso = floatval($caja->caja_restaurante_ingreso);
            $caja_hostal_egreso = floatval($caja->caja_hostal_egreso);
            $caja_restaurante_egreso = floatval($caja->caja_restaurante_egreso);
            $caja->total_sum = $caja_hostal_ingreso + $caja_restaurante_ingreso - $caja_hostal_egreso - $caja_restaurante_egreso;
        }
    ...
    public function registrar(Caja $caja,Request $request){
        $novedades = LibroNovedade::latest()->first();
        $contar = LibroNovedade::count();
        if($contar > 0){
            $username = User::findOrFail($novedades->user_id);
        }else{
            $username = Null;
        }

        $controles = $novedades->controles;
        ...
        return view('admin.caja.registrar',compact('resultadoscontroles','resultadosllaves','contar','username','novedades','currentRoute','cajagrande','detallecajas','caja','codigos','articulos','detalleIngreso','detalleEgreso','total','iniciohostal'));
    }

    public function registrar_restaurante(Caja $caja,Request $request){
        $deliveris = Deliveri::get();
        ...
        ->join('codigo_cajas', 'codigo_cajas.id', '=', 'detalle_cajas.codigo_caja_id')
        ->where('detalle_cajas.caja_id', '=', $caja->id)
        ->where('detalle_cajas.codigo_caja_id', '=', 1)->orderBy('detalle_cajas.id', 'asc')->get();

        $cajagrande = DetalleCaja::select('Egreso')
        ->where('detalle_cajas.caja_id', '=', $caja->id)
        ->where('detalle_cajas.codigo_caja_id', '=', 1)
        ->where('detalle_cajas.articulo_caja_id', '=', 29)
        ->sum('Egreso');
        ...
        /*comandas deuda*/
        $deliveris = Deliveri::where('concluido','FALSE')->with('detalledeliveri')->get();
        $comandas = Comanda::where('Estado','FALSE')->get();
        //return response()->json($cajagrande);
        return view('admin.caja.registrar_restaurante',compact('comandas','deliveris','currentRoute','cajagrande','detallecajas','caja','codigos','articulos','detalleIngreso','detalleEgreso','total','iniciorestaurante'));
    }
    ...
    public function registrar_deposito(Caja $caja,Request $request){
        ...
        $detallecajas = DetalleCaja::select('detalle_cajas.id','detalle_cajas.caja_id','detalle_cajas.codigo_caja_id','detalle_cajas.articulo_caja_id','detalle_cajas.Articulo_description',
        'detalle_cajas.Ingreso','detalle_cajas.Egreso','detalle_cajas.Fecha_registro','detalle_cajas.Factura','detalle_cajas.created_at','detalle_cajas.updated_at',
        'articulo_cajas.Nombre_Articulo','articulo_cajas.Codigo_caja','user_id','codigo_cajas.Nombre')
        ->join('articulo_cajas', 'articulo_cajas.id', '=', 'detalle_cajas.articulo_caja_id')
        ->join('cajas', 'cajas.id', '=', 'detalle_cajas.caja_id')
        ->join('codigo_cajas', 'codigo_cajas.id', '=', 'detalle_cajas.codigo_caja_id')
        ->where('detalle_cajas.caja_id', '=', $caja->id)
        ->where('detalle_cajas.codigo_caja_id', '=', 4)->orderBy('detalle_cajas.created_at', 'asc')->get();
    ...
    public function storedetallecajacerrar(Request $request){
        $user = Auth::user();
        $username = $user->name;
        ...
        $detallecajas = DetalleCaja::create([
            'caja_id' => $request->input('caja_id'),
            'codigo_caja_id' => 1,
            'Articulo_description' => 'CAJA CERRADA POR '.$username.' a '.$hora,
            'Factura' => 'vacio',
            'NFactura' => 0,
            'Ingreso' => 0,
            'Egreso' => 0,
        ...
        notify()->success('Su Cierre De Caja Fue Correctamente') or notify()->success('Su Cierre De Caja Fue Correctamente ⚡️', 'Su Cierre De Caja Fue Correctamente');
        return back();
    }
}
